...
function UserMenuPopover({ open, shown, onClose, userLabel, userEmail, currentUserId, avatarUrl }: { open: boolean; shown: boolean; onClose: () => void; userLabel?: string; userEmail?: string | null; currentUserId: string; avatarUrl?: string | null; }) {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1200, isolation: "isolate" }}>
      ...
      <div style={{ position: "absolute", top: 14, right: 16, width: 280, maxWidth: "calc(100vw - 24px)", borderRadius: 22, border: "1px solid rgba(255,255,255,0.10)", background: "linear-gradient(180deg, rgba(28,28,31,0.98) 0%, rgba(18,19,22,0.98) 100%)", boxShadow: "0 24px 70px rgba(0,0,0,0.44)", overflow: "hidden", transform: shown ? "translateY(0) scale(1)" : "translateY(-6px) scale(0.98)", opacity: shown ? 1 : 0, transformOrigin: "top right", transition: "transform 180ms ease, opacity 180ms ease", backdropFilter: "blur(18px)" }}>
        <div style={{ padding: 14, display: "flex", gap: 10, alignItems: "center", background: "rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          <img src={avatarSrc} alt="Benutzerfoto" className="shrink-0 rounded-xl border border-white/10 object-cover" style={{ width: 42, height: 42 }} />
...
export function TopNav({ userLabel, userEmail, avatarUrl, rightSlot, tenantId, currentUserId, reminderCount = 0, waitlistCount = 0, googleSetupAlertCount = 0 }: { userLabel?: string; userEmail?: string | null; avatarUrl?: string | null; rightSlot?: React.ReactNode; tenantId: string | null; currentUserId: string; reminderCount?: number; waitlistCount?: number; googleSetupAlertCount?: number; }) {
...
>
  <div className="flex h-full flex-col px-2 pb-3">
    ...
    <UserMenuPopover open={userMenuOpen} shown={userMenuShown} onClose={closeUserMenu} userLabel={userLabel} userEmail={userEmail} currentUserId={currentUserId} avatarUrl={avatarUrl} />
  </>
);
}