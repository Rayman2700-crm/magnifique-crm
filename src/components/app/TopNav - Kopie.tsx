      ...
      <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: "min(220px, calc(100vw - 20px))", transform: shown ? "translateX(0)" : "translateX(-24px)", opacity: shown ? 1 : 0, transition: "transform 220ms ease, opacity 220ms ease", borderRight: "1px solid rgba(255,255,255,0.04)", background: "linear-gradient(180deg, rgba(255,250,244,0.04) 0%, rgba(255,248,240,0.012) 100%)", color: "white", boxShadow: "0 24px 72px rgba(0,0,0,0.24)", backdropFilter: "blur(22px) saturate(135%)", display: "flex", flexDirection: "column" }}>
        <div className="flex items-center justify-between border-b border-white/10 p-4">
          <div>
            ...
            <div className="mt-1 text-base font-semibold text-white">{appBranding.appName}</div>
          </div>
...
export function TopNav({ userLabel, userEmail, avatarUrl, avatarRingColor, rightSlot, tenantId, currentUserId, reminderCount = 0, waitlistCount = 0, googleSetupAlertCount = 0 }: { userLabel?: string; userEmail?: string | null; avatarUrl?: string | null; avatarRingColor?: string | null; rightSlot?: React.ReactNode; tenantId: string | null; currentUserId: string; reminderCount?: number; waitlistCount?: number; googleSetupAlertCount?: number; }) {
  ...
  const previousCommunicationUnreadCount = useRef(0);
  ...
  const communicationUnreadCount = Math.max(0, teamUnreadCount + customerUnreadCount);
  ...
  useEffect(() => {
    ...
    function countUnreadTeamMessagesFromRows(rows: any[], lastReadMessageId: string | null) {
      const messages: TeamCounterMessage[] = rows
        .map((row: any): TeamCounterMessage => ({
          id: String(row?.id ?? ""),
          senderId: String(row?.sender_id ?? row?.senderId ?? ""),
          createdAt: String(row?.created_at ?? row?.createdAt ?? ""),
        }))
        .filter((message: TeamCounterMessage) => Boolean(message.id))
        .sort((a: TeamCounterMessage, b: TeamCounterMessage) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    ...
    async function loadTeamUnreadDirect(lastReadMessageId: string | null) {
      try {
        const res = await fetch("/api/chat/messages", { cache: "no-store" });
        if (!res.ok) return { unreadCount: 0, latestMessageId: null as string | null };
        ...
        const messages: TeamCounterMessage[] = rows
          ...
          .sort((a: TeamCounterMessage, b: TeamCounterMessage) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        return {
          unreadCount: countUnreadTeamMessagesFromRows(rows, lastReadMessageId),
          latestMessageId: messages[messages.length - 1]?.id ?? null,
        };
      } catch {
        return { unreadCount: 0, latestMessageId: null as string | null };
      }
    }

    async function loadCommunicationCounts() {
      try {
        const lastReadMessageId = storageKey && typeof window !== "undefined" ? window.localStorage.getItem(storageKey) : null;
        ...
        let rawCustomerUnread = 0;
        let rawTeamUnread = 0;

        if (res.ok) {
          const json = await res.json();
          rawCustomerUnread = Math.max(0, Math.trunc(Number(json?.customerUnreadCount ?? 0)));
          rawTeamUnread = Math.max(0, Math.trunc(Number(json?.teamUnreadCount ?? 0)));
        }

        const directTeamUnread = await loadTeamUnreadDirect(lastReadMessageId);
        rawTeamUnread = Math.max(rawTeamUnread, directTeamUnread.unreadCount);

        const teamTabOpen = pathname?.startsWith("/kommunikation") && searchParams?.get("tab") === "team";
        const legacyChatOpen = pathname?.startsWith("/dashboard/chat") || searchParams?.get("openChat") === "1";

        if ((teamTabOpen || legacyChatOpen) && directTeamUnread.latestMessageId && storageKey && typeof window !== "undefined") {
          const previousLastRead = window.localStorage.getItem(storageKey);
          if (previousLastRead !== directTeamUnread.latestMessageId) {
            window.localStorage.setItem(storageKey, directTeamUnread.latestMessageId);
            window.dispatchEvent(new Event("communication:counts-changed"));
          }
        }

        const nextTeamUnread = teamTabOpen || legacyChatOpen ? 0 : rawTeamUnread;
        const nextCustomerUnread = rawCustomerUnread;
        const nextTotal = nextCustomerUnread + nextTeamUnread;

        if (nextTotal > previousCommunicationUnreadCount.current) {
          setChatPulse(true);
          window.setTimeout(() => setChatPulse(false), 3000);
        }
        previousCommunicationUnreadCount.current = nextTotal;

        if (!cancelled) {
          setCustomerUnreadCount(nextCustomerUnread);
          setTeamUnreadCount(nextTeamUnread);
        }
      } catch {
        const lastReadMessageId = storageKey && typeof window !== "undefined" ? window.localStorage.getItem(storageKey) : null;
        const fallbackTeamUnread = await loadTeamUnreadDirect(lastReadMessageId);
        const teamTabOpen = pathname?.startsWith("/kommunikation") && searchParams?.get("tab") === "team";
        const legacyChatOpen = pathname?.startsWith("/dashboard/chat") || searchParams?.get("openChat") === "1";
        if ((teamTabOpen || legacyChatOpen) && fallbackTeamUnread.latestMessageId && storageKey && typeof window !== "undefined") {
          window.localStorage.setItem(storageKey, fallbackTeamUnread.latestMessageId);
        }
        if (!cancelled) {
          setCustomerUnreadCount(0);
          setTeamUnreadCount(teamTabOpen || legacyChatOpen ? 0 : fallbackTeamUnread.unreadCount);
        }
      }
    }
    ...
    const interval = window.setInterval(() => {
      if (document.visibilityState === "visible") loadCommunicationCounts();
    }, 3000);
    const onFocus = () => loadCommunicationCounts();
    const onStorage = (event: StorageEvent) => {
      if (event.key === storageKey) loadCommunicationCounts();
    };
    const onCountsChanged = () => loadCommunicationCounts();
  ...
  useEffect(() => {
    const baseTitle = appBranding.shortName || appBranding.appName;
      ...
      >
        ...
        >
          <span className="flex h-[46px] min-w-0 flex-col justify-center leading-tight">
            <span suppressHydrationWarning className="block truncate text-[14px] font-semibold tracking-[0.01em] text-[#f4eadc]">{appBranding.appName}</span>
