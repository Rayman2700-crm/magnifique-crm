...
export default function DashboardCalendarCardClient({
  ...
  const loadAppointments = useCallback(async () => {
    ...
    try {
      ...
      const apptRes = await fetch(`/api/dashboard/appointments?${params.toString()}`, {
        method: "GET",
        credentials: "include",
        cache: "no-store",
      });
      ...
      if (!apptRes.ok) {
        setErrorText(apptJson?.error ?? "Kalender konnte nicht geladen werden.");
        setIsInitialLoading(false);
        setIsRefreshing(false);
        return;
      }

      const appts = (apptJson?.items ?? []) as ApptApiRow[];

      const pairKeys = Array.from(new Set(appts.map((a) => `${a.tenant_id}::${a.person_id}`)));
      const cpMap = new Map<string, string>();

      if (pairKeys.length > 0) {
        const cpRes = await fetch(`/api/dashboard/customer-profiles?pairs=${encodeURIComponent(pairKeys.join(","))}`, {
          method: "GET",
          credentials: "include",
          cache: "no-store",
        });
        ...
        if (cpRes.ok) {
          for (const cp of ((cpJson?.items ?? []) as CustomerProfileApiRow[])) {
      ...
      const mappedItems: Item[] = appts.map((a) => {
        ...
        return {
          ...
          tenantName: a.tenantName ?? "Behandler",
          customerProfileId,
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() => setCalendarState({ view: "week", anchorISO: toLocalISODate(new Date()) })}
            >
              Kalender öffnen
            </Button>
          </div>
