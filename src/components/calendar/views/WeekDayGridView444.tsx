...
function getStartTimeOnly(v: string) {
  const normalized = String(v || "").replace("–", "-");
  return normalized.split("-")[0]?.trim() || normalized;
}

function getSmartCompactTime(v: string, widthPx: number) {
  if (widthPx < 96) return getStartTimeOnly(v);
  if (widthPx < 118) return fmtShortTime(v);
  return String(v || "");
}
