...
export default function WeekDayGridView(props: {
  ...
  const dragPreview = useMemo(() => {
    ...
    const baseTheme = luxuryTheme(tenantTheme(payload.tenantName));
    const badge = getReminderBadge(payload.reminderSentAt);
    const previewHeight = Math.max(payload._height ?? 0, 62);
    const previewTop = clampNumber((payload._top ?? 0) + dragging.deltaY, 0, Math.max(0, totalHeight - previewHeight));
    const previewLeftPct = ((payload._col ?? 0) / Math.max(1, payload._cols ?? 1)) * 100;
    const previewWidthPct = 100 / Math.max(1, payload._cols ?? 1);
    ...
    return {
      ...
      leftPct: previewLeftPct,
      widthPct: previewWidthPct,
      targetIso: dragging.targetIso || dragging.iso,
    };
  }, [dragging, tenantTheme, totalHeight]);
            ...
            >
              ...
              >
                ...
                {(view === "week" ? weekDays.map((d) => d.iso) : [anchorISO]).map((iso, idx) => {
                  ...
                  const dropTint: React.CSSProperties = dragPreview?.targetIso === iso ? { boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.08), inset 0 0 0 9999px rgba(255,255,255,0.025)" } : {};
                  ...
                  return (
                    ...
                    >
                      ...
                      {clusterInfo.clusters.map((cluster) => {
                        const baseTheme = tenantTheme(cluster.dominantTenantName);
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              left: 6,
                              width: "calc(100% - 12px)",
                              height: Math.max(72, Math.min(108, cluster.height)),
                              borderRadius: 18,
                              background: theme.cardBg,
                              border: `1px solid ${theme.accentLine}`,
                              boxShadow: `0 18px 34px rgba(0,0,0,0.34), 0 0 0 1px rgba(255,255,255,0.03), inset 0 0 20px ${theme.accentSoft}`,
                              textAlign: "left",
                          ...
                          >
                              ...
                              >
                                ...
                                <span
                                  style={{
                                    ...
                                    borderRadius: 999,
                                    padding: "4px 9px",
                                    background: withAlpha(theme.accent, "1F"),
                            ...
                            {isHovered ? (
                              ...
                              >
                                <div style={{ fontSize: 11, fontWeight: 800, color: theme.cardSubtle }}>
                                  ...
                                  {cluster.events.map((ev) => {
                                    ...
                                    return (
                                      <div key={`${cluster.id}-tooltip-${ev.id}`} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                        ...
                                        />
                                        <span style={{ minWidth: 0, fontSize: 11, color: theme.cardText, fontWeight: 700 }}>
                      ...
                      {dragPreview && dragPreview.targetIso === iso ? (
                        <div
                          style={{
                            position: "absolute",
                            top: dragPreview.top,
                            left: `calc(${dragPreview.leftPct}% + 4px)`,
                            width: `calc(${dragPreview.widthPct}% - 8px)`,
                            height: dragPreview.height,
                            borderRadius: 18,
                            background: dragPreview.theme.cardBg,
                            border: `1px dashed ${dragPreview.theme.accentLine}`,
                            boxShadow: `0 26px 54px rgba(0,0,0,0.46), 0 0 0 1px ${dragPreview.theme.accentLine}, inset 0 0 22px ${dragPreview.theme.accentSoft}`,
                            backdropFilter: "blur(10px)",
                            zIndex: 32,
                            pointerEvents: "none",
                            opacity: 0.96,
                        ...
                        >
                          <span
                            style={{
                              ...
                              bottom: 0,
                              width: 6,
                              background: `linear-gradient(180deg, ${dragPreview.theme.accent} 0%, ${dragPreview.theme.accentMid} 100%)`,
                              boxShadow: `0 0 18px ${dragPreview.theme.accentGlow}`,
                            }}
                          />
                          <div style={{ padding: "10px 12px 10px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
                            <div
                              style={{
                                display: "inline-flex",
                                alignItems: "center",
                                ...
                                color: dragPreview.theme.cardSubtle,
                                fontSize: 11,
                                fontWeight: 800,
                              }}
                            >
                              {dragPreview.payload._timeLine}
                            </div>
                            <div style={{ fontSize: 15, fontWeight: 900, color: dragPreview.theme.cardText, lineHeight: 1.08 }}>
                              {dragPreview.payload._customer}
                            </div>
                              ...
                              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                                <span
                                  style={{
                                    ...
                                    height: 7,
                                    borderRadius: 999,
                                    backgroundColor: dragPreview.theme.accent,
                                    boxShadow: `0 0 10px ${dragPreview.theme.accentGlow}`,
                                    flexShrink: 0,
                                  }}
                                />
                                <span style={{ color: dragPreview.theme.cardMuted, fontSize: 10, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                  {dragPreview.payload.tenantName}
                                </span>
                              </div>
                              <span
                                style={{
                                  ...
                                  alignItems: "center",
                                  justifyContent: "center",
                                  background: dragPreview.badge.bg,
                                  color: dragPreview.badge.text,
                                  border: `1px solid ${dragPreview.badge.border}`,
                                  fontSize: 10,
                                  fontWeight: 900,
                                  lineHeight: 1,
                                }}
                              >
                                {dragPreview.badge.tinyLabel}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      {dayEvents
                        ...
                        .map((ev) => {
                          ...
                          const isDraggedFromCard = dragging?.id === ev.id;
                            ...
                            <button
                              ...
                              style={{
                                ...
                                transform: isDragging
                                  ...
                                  : "translate(0px, 0px)",
                                opacity: isDraggedFromCard ? 0.24 : isSaving ? 0.75 : 1,
                                touchAction: "none",
                                display: "flex",
