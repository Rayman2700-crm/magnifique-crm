...
<div
          ...
          >
            <div style={{ minWidth: 0 }}>
              ...
              {!checkoutOpen ? (
                <div style={{ marginTop: 14 }}>
                  ...
                  <div style={{ marginTop: 8, fontSize: 12, color: "rgba(255,255,255,0.46)" }}>
                    {statusPermissionHint ? "Nur zuständiger Behandler oder Admin darf den Status ändern." : isStatusPending ? "Status wird gespeichert..." : "Ein Klick speichert direkt."}
                  </div>
                  {statusError ? <div style={{ marginTop: 6, fontSize: 12, color: "#fca5a5" }}>{statusError}</div> : null}
                </div>
              ) : null}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "stretch", width: 188, maxWidth: "100%" }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-end" }}>
                {!checkoutOpen ? (
                  <button type="button" className={clientiqueButtonClass("dark")} onClick={() => setEditMode((v) => !v)}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {canOpenCustomerProfile ? (
                    selected.customerProfileId ? (
                      <Link href={`/customers/${selected.customerProfileId}?tab=appointments#appointments`}>
                        <button type="button" className={clientiqueButtonClass("primary", true)}>Zum Kundenprofil</button>
                      </Link>
                    ) : (
                      <form action={openCustomerProfileFromAppointment.bind(null, selected.id)}>
                        <input type="hidden" name="returnTo" value={returnTo} />
                        <button type="submit" className={clientiqueButtonClass("primary", true)}>Zum Kundenprofil</button>
                      </form>
                    )
                  ) : (
                    <button type="button" disabled className={disabledActionButtonStyle() + " w-full"}>Zum Kundenprofil</button>
                  )}
                  ...
                  {canStartCheckout ? (
                    <button
                      ...
                      className={clientiqueButtonClass("success", true)}
                    >
                      Abrechnen
                    </button>
                  ) : (
          ...
          <div className="appointment-detail-scroll" style={{ padding: 16, display: "grid", gap: 14, overflow: "auto", scrollbarWidth: "none", msOverflowStyle: "none" }}>
            ...
            {!checkoutOpen ? (
              <>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <div className="text-sm font-semibold text-white">Weitere Aktionen</div>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>ESC schließt das Fenster.</div>
              </>
            ) : null}
          </div>
