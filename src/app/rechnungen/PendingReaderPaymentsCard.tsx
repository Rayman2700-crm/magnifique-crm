...
export default function PendingReaderPaymentsCard({
  ...
  function stopPolling(paymentId: string) {
    ...
    if (timerId) {
      ...
      delete pollTimersRef.current[paymentId];
    }
    delete pollStartedAtRef.current[paymentId];
  }

  function hardReloadToList() {
    if (hardReloadingRef.current) return;
    hardReloadingRef.current = true;
    window.location.href = buildRechnungenHref({
      qRaw,
  ...
  function schedulePolling(paymentId: string, delayMs = 1800) {
    stopPolling(paymentId);
    pollTimersRef.current[paymentId] = window.setTimeout(() => {
      void pollPaymentUntilDone(paymentId);
    }, delayMs);
  }

  async function pollPaymentUntilDone(paymentId: string) {
    const startedAt = pollStartedAtRef.current[paymentId] ?? Date.now();
    pollStartedAtRef.current[paymentId] = startedAt;

    try {
      const json = await checkPaymentStatus(paymentId);
      ...
      setStatusByPayment((prev) => ({
        ...
        [paymentId]:
          receiptNumber
            ? `${paymentStatus} · Beleg ${receiptNumber} erstellt`
            : stripeStatus
              ? `${paymentStatus} · Stripe ${stripeStatus}`
              : paymentStatus,
      }));
      ...
      if (elapsed > 45000) {
        ...
        setStatusByPayment((prev) => ({
          ...prev,
          [paymentId]: "Noch in Verarbeitung – bitte Status erneut prüfen.",
        }));
        return;
      }

      schedulePolling(paymentId, 1800);
    } catch (error: any) {
      stopPolling(paymentId);
      setErrorByPayment((prev) => ({
        ...prev,
        [paymentId]: String(error?.message ?? "Payment-Status konnte nicht geprüft werden."),
      }));
      setStatusByPayment((prev) => ({ ...prev, [paymentId]: "" }));
    } finally {
      setBusyPaymentId((current) => (current === paymentId ? null : current));
    }
  }
  ...
  async function sendToReader(paymentId: string) {
    ...
    try {
      ...
      setStatusByPayment((prev) => ({
        ...prev,
        [paymentId]: `${statusMessage} · Status wird automatisch geprüft…`,
      }));
      pollStartedAtRef.current[paymentId] = Date.now();
      schedulePolling(paymentId, 1200);
    } catch (error: any) {
      ...
      <div className="border-b border-white/8 px-5 py-4 md:px-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="text-xs text-white/55">Server-driven Reader-Block v2</div>
          </div>
        </div>
