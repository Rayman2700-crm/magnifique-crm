...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!readerId) {
      return NextResponse.json({ error: "reader_id fehlt." }, { status: 400 });
    }
    ...
    if (normalizedStatus === "COMPLETED") {
      return NextResponse.json({ error: "Payment ist bereits abgeschlossen." }, { status: 409 });
    }
