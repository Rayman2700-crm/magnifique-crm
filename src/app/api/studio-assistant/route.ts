...
function mapProviderError(status: number, text: string) {
  ...
  if (normalized.includes("insufficient_quota") || normalized.includes("quota") || normalized.includes("billing")) {
    return "Der Studio-Assistent kann gerade nicht antworten, weil für den OpenAI-API-Key kein verfügbares Guthaben oder Kontingent vorhanden ist. Prüfe in der OpenAI Platform Abrechnung, Budget und API-Key.";
  }

  if (status === 429) {
    return "Der Studio-Assistent ist gerade vorübergehend ausgelastet. Bitte versuche es in wenigen Sekunden erneut.";
  }

  if (status >= 500) {
    return "Der KI-Dienst ist gerade vorübergehend nicht erreichbar. Bitte versuche es gleich noch einmal.";
  }

  return "Der Studio-Assistent konnte gerade keine Antwort erzeugen. Bitte prüfe Server-Konfiguration und API-Zugang.";
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: mapProviderError(response.status, errorText),
        },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json({ answer: answer || "Ich konnte gerade keine inhaltliche Antwort erzeugen. Bitte formuliere die Frage etwas konkreter oder versuche es erneut." });
  } catch (error) {
    return NextResponse.json(
      {
