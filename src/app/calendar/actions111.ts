...
export async function setDefaultCalendar(formData: FormData) {
  ...
  if (!calendarId) {
    redirect("/calendar/google?error=" + encodeURIComponent("Bitte einen Kalender auswählen."));
  }
  ...
  if (error) {
    redirect("/calendar/google?error=" + encodeURIComponent("Konnte Standard-Kalender nicht speichern: " + error.message));
  }

  redirect("/calendar/google?success=" + encodeURIComponent("Standard-Kalender gespeichert ✅"));
}
...
export async function createTestEvent() {
  ...
  if (tokErr) {
    redirect("/calendar/google?error=" + encodeURIComponent("Token DB Fehler: " + tokErr.message));
  }

  const calendarId = (tok as any)?.default_calendar_id as string | null;
  if (!calendarId) {
    redirect("/calendar/google?error=" + encodeURIComponent("Bitte zuerst einen Standard-Kalender speichern."));
  }
  ...
  try {
    ...
    redirect("/calendar/google?error=" + encodeURIComponent(e?.message ?? "Test-Event konnte nicht erstellt werden"));
  }

  redirect("/calendar/google?success=" + encodeURIComponent("Test-Event erstellt ✅"));
}
...
export async function createAppointmentQuick(formData: FormData) {
  ...
  const calendarId = (tok as any)?.default_calendar_id as string | null;
  if (!calendarId) {
    redirect(buildRedirectUrl(baseReturnUrl, "error", "Bitte zuerst Standard-Kalender im Google Setup speichern."));
  }
