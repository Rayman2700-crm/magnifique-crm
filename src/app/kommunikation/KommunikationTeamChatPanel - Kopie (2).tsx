        ...
        {users.map((user) => {
          ...
          return (
            <button
              ...
              className={
                "group relative flex h-10 w-10 items-center justify-center rounded-full border text-[11px] font-bold text-white transition active:scale-[0.97] " +
                (mine
                  ? "border-[#d8c1a0]/50 bg-[#d8c1a0]/16 shadow-[0_0_0_2px_rgba(216,193,160,0.08)]"
                  : "border-[#d8c1a0]/18 bg-[#d8c1a0]/[0.055] hover:border-[#d8c1a0]/38 hover:bg-[#d8c1a0]/[0.11]")
              }
...
export default function KommunikationTeamChatPanel({
      ...
      try {
        ...
        if (!error) {
          for (const row of data ?? []) {
            ...
            userMap.set(userId, { userId, fullName });
          }
        }
      } catch (error) {
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden rounded-[26px] border border-[#d8c1a0]/10 bg-black/[0.06]">
      <TeamUserRail
        users={teamUsers}
